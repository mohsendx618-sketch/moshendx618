...
try:
    adb("wait-for-device")
    adb("install", "-r", "dist/Hamyar-CafeNet-1.1.apk")
    adb("logcat", "-c")
    ...
    checks.append("61 packaged guides including 37 tax guides loaded offline")
    ...
    tap(resource="tax_shortcut")
    expect("۳۷ راهنما")
    expect("مالیات؛ اول کدام اظهارنامه یا خدمت را انتخاب کنم؟")
    shot("08-tax-hub-offline")
    checks.append("Tax shortcut shows only the 37 tax guides")
    tap(resource="search_input")
    adb("shell", "input", "text", "taxvat")
    adb("shell", "input", "keyevent", "KEYCODE_ENTER")
    expect("۱ راهنما")
    tax_title = "اظهارنامه پیش‌فرض مالیات بر ارزش افزوده"
    expect(tax_title)
    tap(description="باز کردن «" + tax_title + "»")
    expect(tax_title)
    shot("09-tax-guide-offline")
    tap(text="قبل شروع")
    expect("خروجی این کار")
    tap(text="باز کردن سایت مربوط", scroll=True)
    expect("https://my.tax.gov.ir")
    tap(text="برگشت")
    tap(text="گیر کردم")
    expect("خرید در کارپوشه هست ولی اعتبار نیامده")
    shot("10-tax-troubleshooting-offline")
    tap(text="منابع")
    expect("حدود اعتبار این آموزش")
    expect("۵ شهریور ۱۴۰۵")
    checks.append("Tax search, guide tabs, sources, caveats and official link work offline")
    home()
    tap(resource="tax_shortcut")
    expect("۳۷ راهنما")
    for alias,title in [
        ("tax100","فرم تبصره ماده ۱۰۰؛ مالیات مقطوع مشاغل"),
        ("taxcompany","اظهارنامه عملکرد اشخاص حقوقی؛ شرکت و مؤسسه"),
        ("taxrent","اظهارنامه درآمد اجاره املاک؛ موجر"),
        ("taxsalary","ارسال فهرست مالیات حقوق کارکنان"),
        ("taxbooks","بارگذاری دفاتر تجاری الکترونیکی")
    ]:
        tap(resource="search_input")
        adb("shell", "input", "text", alias)
        adb("shell", "input", "keyevent", "KEYCODE_ENTER")
        expect("۱ راهنما")
        expect(title)
        tap(description="باز کردن «" + title + "»")
        ...
        home()
        tap(resource="tax_shortcut")
    checks.append("Article 100, company, rent, salary and books searches open the correct guides offline")
